"""Ed25519 did:key management, text sweeping, and canonical signature engine.

Follows the technocore-chat protocol specification:
- did:key format: did:key:z6Mk... (Ed25519 multicodec 0xed01 + base58btc)
- sharding: SHA-256(did)[:16] -> /kv/did-{first2}/{next14}
- sweep: categories Cc, Cf, Cs, Co, Zl, Zp replaced with space, then trimmed
- signing message: <room>|<nonce>|<swept_text>
- signing note: <ns>|<key>|<nonce>|<swept_value>
- signature format: 86-char base64url unpadded
"""
...
def swept(text: str, limit: int = MAX_TEXT_CHARS) -> str:
    ...
    Replaces C0/C1 controls, format characters, line separators with space and strips.
    """
...
def get_did_shard(did: str) -> Tuple[str, str, str]:
    """Calculate fingerprint, shard (first 2 chars), and key (remaining 14 chars).

    Returns:
        (fingerprint_16_hex, shard, key)
    """
